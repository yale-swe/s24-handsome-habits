...
jest.mock('../src/services/userService', () => ({
    findUserFromRequest: jest.fn().mockRejectedValue(new Error('User not found')),
}));
...
describe('Login Component', () => {
            it('renders correctly', () => {
                    const { getByText } = render( < Login navigation = {
                            { navigate: jest.fn() } }
                        />);
                        expect(getByText('Welcome to')).toBeTruthy(); expect(getByText('Handsome Habits')).toBeTruthy(); expect(getByText('Sign in with Google')).toBeTruthy(); expect(getByText('Sign in with Yale CAS')).toBeTruthy();
                    });
                ...
                it('calls Google sign-in on button press', async() => {
                        ...
                        const { getByText } = render( < Login navigation = {
                            ...
                            fireEvent.press(googleSignInButton); await waitFor(() => {
