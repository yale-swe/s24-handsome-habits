...
describe('Authentication and logout', () => {
  ...
  it('handles successful login', async () => {
    const mockResponse = { type: 'success', authentication: { accessToken: 'token123' } };
    ...
    expect(Api.post).toHaveBeenCalledWith('/auth/login', { token: 'token123' });
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('@user', JSON.stringify({ user: 'testUser' }));
