...
describe("addHabit", () => {
    ...
    it("successfully adds a new habit", async () => {
        ...
        expect(apiUtil.post).toHaveBeenCalledWith("/habits/add", { habit: newHabit });
        expect(AsyncStorage.setItem).toHaveBeenCalledWith("habit", JSON.stringify(mockResponseData));
        expect(updatePointswithChange).toHaveBeenCalledWith(newHabit.category_name, expect.any(Object));
...
describe("calculatePoints", () => {
    ...
    it("calculates points and coins correctly for low intensity and short duration", () => {
      ...
      it("adds points for medium workout duration and extra coin for medium intensity", () => {
        ...
        // high insensity, medium duration test
